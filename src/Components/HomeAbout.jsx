...
function HomeAbout() {
    ...
    return (
        <div style={{ height: "100%", backgroundColor: "#fff" }} id='about'>
            <Row className='h-100  gx-0 my-5 mx-3' style={{ borderBottom: " 2px  solid #d9efe9", backgroundColor:"#d9efe9"}}>
                <Col lg={6} className='p-5 ' style={{backgroundColor:""}}>
            ...
            <HomeDeptView/>
            <Row className='gx-0 py-5 px-4' style={{ borderBottom: " 2px  solid #d9efe9", borderTop:" 2px  solid #d9efe9"}}>
                <Col xs={12}>
                    <h1 className='text-center fw-bold'>Our Docters</h1>
                </Col>
                <Col xs={12} className='d-flex justify-content-center my-3 '>
                    <input type="text" className='mx-5 w-50 form-control' style={{backgroundColor:"#d9efe9"}} placeholder='Search Docter or Department' onChange={(e)=>{setSearch(e.target.value)}}/>
                </Col>
                {
                    docterList ?
                        docterList.map(item => (
                            <Col md={4} className='p-5 d-flex justify-content-center' key={item._id}>
                                <Card style={{ width: '20rem',  backgroundColor:"#d9efe9"}}>
                                    <Card.Img variant="top" src={`${BASE_URL}/upload/${item.dr_image}`} height={250} />
                                    <Card.Body>
                                        <Card.Title className='text-center fw-bold'>{item.firstname} {item.lastname}</Card.Title>
                                        <Card.Text className='px-4'>
                                            <h6><span className='fw-bold'>Department :</span> {item.department.slice(0,-24)}</h6>
                                        ...
                                        <div className='d-flex justify-content-evenly mt-4 mb-3'>
                                            <Link to={`/${item._id}/booking`} ><Button variant="outlined">More About</Button></Link>
                                            <Link to={`/${item._id}/booking`} ><Button variant="contained" > For Booking</Button></Link>
                                        </div>
                                    </Card.Body>
                                </Card>
                            </Col>
